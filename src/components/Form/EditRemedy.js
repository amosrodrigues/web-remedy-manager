...
import { updateRemedy } from '../../api/remedyApi';
import { StyledEditRemedy } from './styleEditRemedy ';
...
export default function EditRemedy() {
  ...
  const { dataMaker, dataReact, remedyEdit, setToggleShow } =
    useContext(RemedyContext);

  const [remedy, setRemedy] = useState({
    ...remedyEdit,
    dateValid: moment.utc(remedyEdit.dateValid).format('YYYY-MM-DD'),
  });
  ...
  function resetForm() {
    ...
    setOptions(false);
    setToggleShow('medicamentos');
  }

  async function handleEditRemedy(event) {
    event.preventDefault();

    const response = await updateRemedy(remedy);
  ...
  const getReacChecked = useCallback(() => {
    let checkboxes = document.querySelectorAll('input[type="checkbox"]');

    remedyEdit.reacts.forEach((react) => {
      checkboxes.forEach((checkbox) => {
        if (checkbox.value === react) {
  ...
  return (
    <StyledEditRemedy>
      <form id="form-remedy" onSubmit={handleEditRemedy}>
